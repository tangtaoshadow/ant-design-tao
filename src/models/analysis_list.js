...
let model = {
  ...
  state: {
    ...
    // 设置irt为公开
    analysis_list_set_public_status: -1,
    analysis_list_set_public_time: 0
  },
  ...
  effects: {
    ...
    *set_library_public_by_id({ payload }, sagaEffects) {
      ...
      try {
        // 捕获异常
        result = yield call(
          analysis_list_service.set_library_public_by_id,
          payload
        );
      } catch (e) {
        result = "";
  ...
  reducers: {
    ...
    set_library_public_by_id_result(state, { payload: result }) {
      ...
      obj.analysis_list_set_public_time = new Date().getTime();

      // 1 检查 返回数据状态
      if (-1 == res_status) {
        // 发生严重错误
        obj.analysis_list_set_public_status = res_status;
        return obj;
      }
