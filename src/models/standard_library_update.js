...
let model = {
  ...
  state: {
    ...
    standard_library_update_task_id: null,
    // 任务状态 数据 时间
  ...
  effects: {
    ...
    *update_library_list_id_detail({ payload }, sagaEffects) {
      ...
      try {
        result = yield call(
          standard_library_update_service.update_library_list_id_detail,
          payload
        );
      } catch (e) {
        result = "";
    ...
    update_library_list_id_detail_result(state, { payload: result }) {
      ...
      if ("error" != result) {
        try {
          ...
          (obj.standard_library_update_task_id = result.taskId),
      ...
      if (-1 == res_status) {
        // 发生严重错误
        (obj.standard_library_update_task_id = null),
